...
//        checkAuthentication();
    ...
    protected void setViews() {
        ...
        mImageViewEnd.setOnClickListener(view -> {
           // hitEndCallApi(AppSettings.getString(AppSettings.callId));
            end();
        });
    }
...
//        mTextViewUserId.setText(getRemoteNicknameOrUserId(call));
    ...
    private void hitEndCallApi(final String strCallId) {
        JSONObject jsonObject = new JSONObject();
        JSONObject json = new JSONObject();
        try {
            jsonObject.put("callId", strCallId);
            json.put(AppConstants.projectName, jsonObject);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        WebServices.postApi(mActivity, AppUrls.endCall, json, true, true, new WebServicesCallback() {
            @Override
            public void OnJsonSuccess(JSONObject response) {

                parseJsonEndCall(response);
            }
    ...
    private void parseJsonEndCall(JSONObject response) {
        try {
            JSONObject jsonObject = response.getJSONObject(AppConstants.projectName);
            if (jsonObject.getString(AppConstants.resCode).equals("1")) {
                // AppUtils.showToastSort(mActivity, "voice call");
                end();
            } else {
