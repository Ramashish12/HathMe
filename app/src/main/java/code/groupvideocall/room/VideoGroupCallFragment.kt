...
import code.groupvideocall.util.copyText
import code.groupvideocall.util.showToast
import code.groupvideocall.util.toReadableString
import code.groupvoicecall.room.GroupCallFragmentDirections
import com.google.android.flexbox.FlexboxLayoutManager
import com.google.android.flexbox.JustifyContent
import com.hathme.android.R
import com.hathme.android.databinding.FragmentVideoGroupCallBinding
import com.sendbird.calls.AudioDevice
import com.sendbird.calls.SendBirdCall

class VideoGroupCallFragment : Fragment() {
    lateinit var binding: FragmentVideoGroupCallBinding
    lateinit var viewModel: VideoGroupCallViewModel
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_video_group_call, container, false)
        val roomId = (activity as? VideoRoomActivity)?.let {
            if (it.isNewlyCreated() && !isNewlyCreatedRoomInfoShown) {
                showNewlyCreatedRoomInfo(it.getRoomId())
                isNewlyCreatedRoomInfoShown = true
            }

            viewModel = VideoGroupCallViewModel(it.getRoomId())
            it.getRoomId()
    ...
    private fun initRecyclerView() {
        ...
        val adapter = VideoParticipantAdapter(viewModel.participants.value ?: listOf())
